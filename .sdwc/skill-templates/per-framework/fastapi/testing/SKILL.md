...
> This skill defines testing rules for the **{{ name }}** service (FastAPI / Python).
...
- Edge cases: boundary values, empty inputs, max-length inputs, concurrent access.
- Failure cases: invalid input, missing required fields, unauthorized access, resource not found.
- Security cases: injection attempts, token tampering, privilege escalation, rate limit bypass, CORS violations.
{% endif %}
