...
> This skill defines FastAPI-specific patterns for the **{{ name }}** service.
...
**Rules:**
- One session per request (injected via `Depends(get_db)`).
- Services call `commit()`. Repositories only `flush()`.
- Use `expire_on_commit=False` to avoid lazy-load issues after commit.
{% endif %}
