...
{% set ns = namespace(svc_list=[]) %}
{% for svc in services %}
{% set ns.svc_list = ns.svc_list + [svc.name ~ "(" ~ svc.type ~ ")"] %}
{% endfor %}
> **{{ project.name }}** | {{ ns.svc_list | join(", ") }}
> Team: {{ collaboration.human_developers }} human + Claude | Review: {{ collaboration.review_policy }}
> Methodology: {{ process.methodology }} | Branch: {{ version_control.branch_strategy }}
...
**Architecture**: {{ architecture.pattern }} / {{ architecture.internal_style }}
{% else %}
...
**Team**: {{ collaboration.human_developers }} human + Claude
**Review policy**: {{ collaboration.review_policy }}

### Model Configuration

- **Primary**: {{ collaboration.model_routing.primary }}
{% if collaboration.model_routing.secondary %}
- **Secondary**: {{ collaboration.model_routing.secondary }}
{% if collaboration.model_routing.routing_rule %}
- **Routing**: {{ collaboration.model_routing.routing_rule }}
{% endif %}
{% endif %}
{% if collaboration.use_subagent %}
- **Sub-agent**: enabled
{% endif %}
...
{% for ps in collaboration.per_service %}
#### {{ ps.service }} - `{{ ps.mode }}`

{% if ps.claude_roles %}
**I do**: {{ ps.claude_roles | join(", ") }}
{% endif %}
{% if ps.human_roles %}
**Human does**: {{ ps.human_roles | join(", ") }}
{% endif %}
{% if ps.claude_boundaries %}
**DO NOT**: {{ ps.claude_boundaries | join(", ") }}
{% endif %}

**Decision authority**:
- Autonomous: {{ ps.decision_authority.claude_autonomous | join(" | ") }}
- [!] Requires approval: {{ ps.decision_authority.requires_approval | join(" | ") }}
...
{% for rule in collaboration.absolute_rules %}
- {{ rule }}
{% endfor %}
...
- **Branch strategy**: {{ version_control.branch_strategy }} - {{ version_control.branch_strategy_description }}
{% else %}
...
- **PR**: created by {{ version_control.pr_policy.created_by }}, squash merge
{% else %}
- **PR**: created by {{ version_control.pr_policy.created_by }}
{% endif %}
{% endif %}
...
{% if level.coverage_target %}
- {{ level.level }}: {{ level.framework }} (target: {{ level.coverage_target }})
{% else %}
- {{ level.level }}: {{ level.framework }}
{% endif %}
{% endfor %}
...
{% endif %}
{% if svc.type == "worker" %}
| 40-worker-design | Worker specs | Adding/changing workers |
{% endif %}
{% if svc.type == "mobile_app" %}
| 50-mobile-design | Screens, navigation | Mobile work |
{% endif %}
{% if svc.type == "data_pipeline" %}
| 60-pipeline-design | Pipeline specs | Pipeline work |
{% endif %}
...
1. Assess whether the addition keeps total task size within review time (~{{ process.task_review_minutes | default("30") }} minutes).
...
- Work in sprint cycles ({{ process.sprint_length }} weeks).
{% else %}
- Work in sprint cycles.
{% endif %}
- Tasks align with sprint goals.
- Review progress at sprint boundaries.
{% endif %}
{% if process.methodology == "kanban" %}
- Continuous flow. Pull next task only when current is complete.
{% if process.wip_limit %}
- Respect WIP limit: max {{ process.wip_limit }} tasks in progress.
{% endif %}
{% endif %}
{% if process.methodology == "scrumban" %}
- Sprint structure with continuous flow within sprints.
{% if process.wip_limit %}
- Respect WIP limit: max {{ process.wip_limit }}.
{% endif %}
{% endif %}
{% if process.methodology == "xp" %}
- Short iterations with continuous feedback.
- Pair programming when in collaborative mode.
{% endif %}
